...
@section('title', __('app.books.edit_book'))
...
<div class="px-4 max-w-2xl mx-auto">
    <div class="mb-6">
        <a href="{{ route('books.show', $book) }}" class="text-indigo-600 hover:text-indigo-700 flex items-center">
            ...
            {{ __('app.books.back_to_book') }}
        </a>
    </div>

    <div class="bg-white rounded-lg shadow-lg p-8">
        <h1 class="text-3xl font-bold text-gray-900 mb-8">{{ __('app.books.edit_book') }}</h1>
                ...
                <template x-for="(field, key) in data?.fetched" :key="key">
                    <div x-show="field && field !== data.current[key]">
                        ...
                        <label x-show="key === 'cover_url'" class="flex items-start gap-2 p-2 bg-white rounded">
                            ...
                            <div class="flex-1">
                                <strong class="text-sm capitalize">{{ __('app.books.cover_image_label') }}:</strong>
                                <div class="flex gap-4 mt-2">
                                    <div class="text-center">
                                        <div class="text-xs text-gray-600 mb-1">{{ __('app.books.current_label') }}</div>
                                    ...
                                    <div class="text-center">
                                        <div class="text-xs text-green-600 font-medium mb-1">{{ __('app.books.new_from_api') }}</div>
        ...
        <form method="POST" action="{{ route('books.update', $book) }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label for="series" class="block text-sm font-medium text-gray-700">{{ __('app.books.series') }}</label>
                ...
                <div>
                    <label for="series_position" class="block text-sm font-medium text-gray-700">{{ __('app.books.series_position') }}</label>
            ...
            <div>
                ...
                <input type="text" name="language" id="language" maxlength="10" placeholder="{{ __('app.books.language_placeholder') }}" value="{{ old('language', $book->language) }}"
            ...
            <div>
                <label for="added_at" class="block text-sm font-medium text-gray-700">{{ __('app.books.added') }}</label>
                <input type="date" name="added_at" id="added_at" value="{{ old('added_at', $book->added_at ? $book->added_at->format('Y-m-d') : '') }}"
                       class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 @error('added_at') border-red-500 @enderror">
                @error('added_at')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label for="started_at" class="block text-sm font-medium text-gray-700">{{ __('app.books.started_reading') }}</label>
                ...
                <div>
                    <label for="finished_at" class="block text-sm font-medium text-gray-700">{{ __('app.books.finished_reading') }}</label>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('app.books.cover_images') }}</label>
                <p class="text-sm text-gray-500 mb-4">{{ __('app.books.covers_managed_below') }}</p>
            </div>

            <div>
                <label for="cover_url" class="block text-sm font-medium text-gray-700">{{ __('app.books.cover_url_label') }}</label>
                ...
                <p class="text-xs text-gray-500 mt-1">{{ __('app.books.cover_url_help') }}</p>
                @error('cover_url')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                <label for="thumbnail" class="block text-sm font-medium text-gray-700">{{ __('app.books.thumbnail_url_label') }}</label>
                ...
                <p class="text-xs text-gray-500 mt-1">{{ __('app.books.thumbnail_url_help') }}</p>
                @error('thumbnail')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="status" id="status" required
                    ...
                    <option value="want_to_read" {{ old('status', $book->status) === 'want_to_read' ? 'selected' : '' }}>{{ __('app.books.want_to_read') }}</option>
                    <option value="currently_reading" {{ old('status', $book->status) === 'currently_reading' ? 'selected' : '' }}>{{ __('app.books.currently_reading') }}</option>
                    <option value="read" {{ old('status', $book->status) === 'read' ? 'selected' : '' }}>{{ __('app.books.read') }}</option>
                </select>
                @error('status')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                <label for="format" class="block text-sm font-medium text-gray-700">{{ __('app.books.format') }}</label>
                <select name="format" id="format"
                    ...
                    <option value="">{{ __('app.books.select_format') }}</option>
                    <option value="digital" {{ old('format', $book->format) === 'digital' ? 'selected' : '' }}>{{ __('app.books.format_digital') }}</option>
                    <option value="paperback" {{ old('format', $book->format) === 'paperback' ? 'selected' : '' }}>{{ __('app.books.format_paperback') }}</option>
                    <option value="hardcover" {{ old('format', $book->format) === 'hardcover' ? 'selected' : '' }}>{{ __('app.books.format_hardcover') }}</option>
                    <option value="audiobook" {{ old('format', $book->format) === 'audiobook' ? 'selected' : '' }}>{{ __('app.books.format_audiobook') }}</option>
                    <option value="magazine" {{ old('format', $book->format) === 'magazine' ? 'selected' : '' }}>{{ __('app.books.format_magazine') }}</option>
                    <option value="spiral_bound" {{ old('format', $book->format) === 'spiral_bound' ? 'selected' : '' }}>{{ __('app.books.format_spiral_bound') }}</option>
                    <option value="leather_bound" {{ old('format', $book->format) === 'leather_bound' ? 'selected' : '' }}>{{ __('app.books.format_leather_bound') }}</option>
                    <option value="journal" {{ old('format', $book->format) === 'journal' ? 'selected' : '' }}>{{ __('app.books.format_journal') }}</option>
                    <option value="comic" {{ old('format', $book->format) === 'comic' ? 'selected' : '' }}>{{ __('app.books.format_comic') }}</option>
                    <option value="graphic_novel" {{ old('format', $book->format) === 'graphic_novel' ? 'selected' : '' }}>{{ __('app.books.format_graphic_novel') }}</option>
                    <option value="manga" {{ old('format', $book->format) === 'manga' ? 'selected' : '' }}>{{ __('app.books.format_manga') }}</option>
                    <option value="box_set" {{ old('format', $book->format) === 'box_set' ? 'selected' : '' }}>{{ __('app.books.format_box_set') }}</option>
                    <option value="omnibus" {{ old('format', $book->format) === 'omnibus' ? 'selected' : '' }}>{{ __('app.books.format_omnibus') }}</option>
                    <option value="reference" {{ old('format', $book->format) === 'reference' ? 'selected' : '' }}>{{ __('app.books.format_reference') }}</option>
                    <option value="other" {{ old('format', $book->format) === 'other' ? 'selected' : '' }}>{{ __('app.books.format_other') }}</option>
                </select>
                @error('format')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="grid grid-cols-3 gap-4">
                <div class="col-span-1">
                    <label for="purchase_date" class="block text-sm font-medium text-gray-700">{{ __('app.books.purchase_date') }}</label>
                ...
                <div class="col-span-1">
                    <label for="purchase_price" class="block text-sm font-medium text-gray-700">{{ __('app.books.purchase_price') }}</label>
                ...
                <div class="col-span-1">
                    <label for="purchase_currency" class="block text-sm font-medium text-gray-700">{{ __('app.books.currency') }}</label>
            ...
            <div>
                <label for="current_page" class="block text-sm font-medium text-gray-700">{{ __('app.books.current_page') }}</label>
        ...
        <div class="mt-8 pt-8 border-t border-gray-200">
            <h2 class="text-2xl font-bold text-gray-900 mb-6">{{ __('app.books.cover_images') }}</h2>

            <!-- Multiple Covers Gallery -->
            @if($book->covers->count() > 0)
            <div class="mb-8">
                <p class="text-sm text-gray-600 mb-3">{{ __('app.books.current_covers') }}</p>
            ...
            <div class="bg-gray-50 rounded-lg p-6">
                <form action="{{ route('books.covers.upload', $book) }}" method="POST" enctype="multipart/form-data" id="cover-upload-form">
                    @csrf
                    <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('app.books.upload_new_cover') }}</label>
                    ...
                    <div id="drop-zone-covers" class="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-indigo-500 transition-colors cursor-pointer bg-white">
                        ...
                        <p class="mt-2 text-sm text-gray-600">
                            <span class="font-semibold text-indigo-600">{{ __('app.books.click_to_upload') }}</span> {{ __('app.books.drag_and_drop') }}
                        </p>
                        <p class="text-xs text-gray-500 mt-1">{{ __('app.books.image_requirements') }}</p>
    ...
    function handleFiles(files) {
        if (files.length > 0) {
            ...
            if (!allowedTypes.includes(file.type)) {
                alert(@json(__('app.books.invalid_image_type')));
                fileInput.value = '';
                return;
            }
            ...
            if (file.size > 25 * 1024 * 1024) {
                alert(@json(__('app.books.file_too_large')));
                fileInput.value = '';
                return;
            }

            // Display file name
            fileNameDisplay.textContent = '✓ ' + @json(__('app.books.selected_file')) + ': ' + file.name;
...
function deleteCover(coverId) {
    if (!confirm(@json(__('app.books.delete_cover_confirm')))) {
        return;
    }
    ...
    function handleCoverFiles(files) {
        if (files.length > 0) {
            ...
            if (invalidFiles.length > 0) {
                alert(@json(__('app.books.invalid_files')) + ':\n' + invalidFiles.join('\n'));
            }

            if (validCount > 0) {
                // Display file names
                if (validCount === 1) {
                    fileNameDisplayCovers.textContent = '✓ ' + @json(__('app.books.selected_file')) + ': ' + files[0].name;
                } else {
                    fileNameDisplayCovers.textContent = '✓ ' + @json(__('app.books.selected_file')) + ': ' + validCount + ' file(s)';
                }
